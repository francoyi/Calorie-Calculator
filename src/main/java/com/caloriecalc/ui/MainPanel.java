...
import javax.swing.*; import java.awt.*; import java.nio.file.Path; import java.time.LocalDate; import java.time.ZoneId;
public class MainPanel extends JPanel {
  ...
  public MainPanel(){
    ...
    JPanel nav = new JPanel(new FlowLayout(FlowLayout.LEFT)); nav.add(prevBtn); nav.add(todayBtn); nav.add(nextBtn); top.add(nav, BorderLayout.WEST);
    dateLabel.setFont(dateLabel.getFont().deriveFont(Font.BOLD, 16f)); top.add(dateLabel, BorderLayout.CENTER);
    JPanel actions = new JPanel(new FlowLayout(FlowLayout.RIGHT)); actions.add(addMealBtn); top.add(actions, BorderLayout.EAST);
    add(top, BorderLayout.NORTH); add(new JScrollPane(dailyPanel), BorderLayout.CENTER);
  ...
  private void onAddMeal(){ MealDialog dlg = new MealDialog(SwingUtilities.getWindowAncestor(this), service, current, null); dlg.setVisible(true); refresh(); }
  private void refresh(){ dateLabel.setText(current.toString()); DailyLog d = service.getDay(current); dailyPanel.renderDay(d); }
}
