...
public class FoodLogService {
  ...
  public DailyLog getDay(LocalDate date){
    ...
    if (d==null){ d = new DailyLog(date); d.setMeals(new ArrayList<>()); d.setTotalKcal(0.0); }
    return d;
  }
  public Meal newEmptyMeal(LocalDate date, String label){ return new Meal(UUID.randomUUID().toString(), date, label); }
  ...
  public MealEntry resolveEntryFromInput(String input){
    ...
    if (per100 != null && per100.energyKcal()!=null){
      ...
      kcalServing = item.getKcalPerServing();
  ...
  public void saveMeal(LocalDate date, Meal meal){
    ...
    if (day==null){ day = new DailyLog(date); day.setMeals(new ArrayList<>()); }
    boolean replaced=false;
    for (int i=0;i<day.getMeals().size();i++){
      if (day.getMeals().get(i).getId().equals(meal.getId())){ day.getMeals().set(i, meal); replaced=true; break; }
    }
