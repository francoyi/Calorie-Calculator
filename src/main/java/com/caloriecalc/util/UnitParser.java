...
public final class UnitParser {
  ...
  public static Parsed parse(String input){
    ...
    String norm = input.trim().toLowerCase(Locale.ROOT).replaceAll("\s+", " ");
    Matcher m = AMOUNT_UNIT.matcher(norm);
    double amount = Double.NaN; String unit = null;
    if (m.find()){ amount = Double.parseDouble(m.group(1)); unit = m.group(2).toLowerCase(Locale.ROOT); }
